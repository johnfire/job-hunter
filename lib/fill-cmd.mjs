...
import { detectModel } from "./ai-detect.mjs";
import { captureToBrain } from "./brain-client.mjs";
...
export async function runFill(url, answersPath) {
  ...
  // Detect AI model for tracking
  const aiModel = detectModel();
  ...
  // Ashby shows JD first — click Apply to reveal the form
  if (detectPlatform(url) === "ashby") {
    const applyBtn = page
      .locator(
  ...
  // Save to Open Brain with model attribution
  const company = new URL(url).hostname.replace('.jobs.', '-').replace('.personio.de', '').replace('www.', '');
  await captureToBrain(
    `Filled ${filled} fields for ${company} application form`,
    'career-ops',
    aiModel,
    'task'
  );
