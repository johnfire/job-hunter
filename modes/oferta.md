...
When the user pastes a job offer (text or URL), ALWAYS deliver all 7 blocks (A-F evaluation + G legitimacy):
...
Table with data and cited sources. If no data is available, say so — never invent.
...
(only if score >= 4.5 — draft answers for application form questions)
...
Write one TSV file to `batch/tracker-additions/{###}-{company-slug}.tsv` — NEVER edit `data/applications.md` directly.

**TSV format (9 tab-separated columns):**
