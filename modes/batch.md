...
1. **Read state**: `batch/batch-state.tsv` → identify what's already processed
...
- `--dry-run` — list pending offers without executing
...
- If conductor dies → re-run → reads `batch-state.tsv` → skips completed
...
| Error                 | Recovery                                                             |
| --------------------- | -------------------------------------------------------------------- |
| URL inaccessible      | Worker fails → conductor marks `failed`, moves to next               |
